...
- **Requirement basis:** ZT-71, ZT-31; SRS 1412–1415 (expected launch digests in TRAIN)
- **Related:** ADR-0010 — session attestation through `cmcd`
...
| 2 — session | Attestation reports produced live, bound to a TLS session | `cmcd`, on peer connection (ADR-0010) |
...
- **Not `rootfsSha256`,** although that is the field CMC's own verifier matches reference values on,
  ...
  across the spike's three tags: a rootfs change and a config-only change give the same
  `rootfsSha256` (`de19a1d3…` for both) and different template hashes (`0842477b…` against
  `b47b2a54…`). A verifier holding the previous `rootfsSha256` from TRAIN would accept a peer
...
- SRS, Annex A — ZT-31, ZT-71, ZT-10, ZT-11
- SRS 1412–1415 — expected launch digests in TRAIN
